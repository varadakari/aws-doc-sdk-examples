...
class DirectIOPartStreamBuf : public std::streambuf {
    int fd;
    off_t offset;
    size_t totalSize;
    size_t readPos = 0;
...
public:
    DirectIOPartStreamBuf(int fd_, off_t offset_, size_t size_, size_t blockSize_ = ALIGNMENT)
        : fd(fd_), offset(offset_), totalSize(size_), blockSize(blockSize_) {
        bufSize = blockSize * 16; // 64KB read buffer
        if (posix_memalign((void**)&alignedBuf, blockSize, bufSize) != 0) {
            throw std::runtime_error("Failed to allocate aligned buffer");
        }
        setg(alignedBuf, alignedBuf, alignedBuf);
    }

    ~DirectIOPartStreamBuf() {
        free(alignedBuf);
    }

protected:
    int_type underflow() override {
        if (readPos >= totalSize) return traits_type::eof();

        size_t toRead = std::min(bufSize, totalSize - readPos);
        size_t alignedRead = (toRead + blockSize - 1) & ~(blockSize - 1);

        ssize_t n = pread(fd, alignedBuf, alignedRead, offset + readPos);
        if (n <= 0) return traits_type::eof();

        size_t actual = std::min((size_t)n, toRead);
        readPos += actual;

        setg(alignedBuf, alignedBuf, alignedBuf + actual);
        return traits_type::to_int_type(*gptr());
    }
};

class DirectIOPartStream : public Aws::IOStream {
    DirectIOPartStreamBuf buf;

public:
    DirectIOPartStream(int fd, off_t offset, size_t size)
        : Aws::IOStream(nullptr), buf(fd, offset, size) {
        rdbuf(&buf);
    }
};
                       ...
                       size_t threadCount)
{
    ...
    lseek(fd, 0, SEEK_SET);
    size_t totalParts = (fileSize + PART_SIZE - 1) / PART_SIZE;

    for (size_t t = 0; t < threadCount; ++t) {
        workers.emplace_back([&, t]() {
            while (true) {
                size_t partNumber = nextPart.fetch_add(1);
                if (partNumber >= totalParts) break;

                off_t offset = partNumber * PART_SIZE;
                size_t partSize = std::min<size_t>(PART_SIZE, fileSize - offset);

                try {
                    auto stream = Aws::MakeShared<DirectIOPartStream>("UploadStream", fd, offset, partSize);
                    UploadPartRequest request;
                    request.SetBucket(bucket);
                    request.SetKey(key);
                    request.SetUploadId(uploadId);
                    request.SetPartNumber(static_cast<int>(partNumber + 1));
                    request.SetBody(stream);
                    request.SetContentLength(partSize);

                    auto outcome = client->UploadPart(request);
                    if (outcome.IsSuccess()) {
                        PartETag partETag{ static_cast<int>(partNumber + 1), outcome.GetResult().GetETag() };
                        {
                        ...
                        std::cerr << "UploadPart " << partNumber + 1 << " failed: " << outcome.GetError().GetMessage() << std::endl;
                    }
